...
<head>
    ...
    <style>
        /* Same base styles as viewSafari.jsp but with blue theme */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Inter', sans-serif; background: #f7faf7; color: #1a2e1a; min-height: 100vh; }
        ...
        .stay-card:hover::before { transform: scaleX(1); }
        ...
        .stay-card:hover .card-image { transform: scale(1.03); }
        .stay-card .card-image-wrapper { position: relative; overflow: hidden; }
        ...
        .stay-card .card-body { padding: 24px; }
        ...
        .stay-card .card-body .card-price .amount {
            font-size: 1.6rem;
            font-weight: 800;
            color: #2563eb;
        }
        .stay-card .card-body .card-price .per {
            font-size: 0.85rem;
            color: #6b7a6b;
            font-weight: 500;
        }
        .stay-card .card-body .card-actions { display: flex; gap: 12px; }
        ...
        /* Delete Modal (same as viewSafari) */
        ...
        .delete-modal .modal-icon i { animation: shakeIcon 0.6s ease-in-out; }
        @keyframes shakeIcon {
        ...
        .empty-state .empty-icon { font-size: 4rem; color: #dce4dc; margin-bottom: 16px; }
        .empty-state h3 { color: #1a2e1a; font-weight: 700; margin-bottom: 8px; }
        .empty-state p { color: #6b7a6b; }
        .empty-state .btn-add {
            display: inline-flex;
            align-items: center;
            gap: 10px;
        ...
        @media (max-width: 992px) {
            ...
            .stay-grid .grid-wrapper { grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 24px; }
        }
        @media (max-width: 768px) {
            .page-header { padding: 30px 0 24px; }
            .page-header h1 { font-size: 1.8rem; }
            .stats-bar .stats-wrapper { gap: 30px; }
            .stats-bar .stat-item .stat-number { font-size: 1.6rem; }
            .stay-grid .grid-wrapper { grid-template-columns: 1fr 1fr; gap: 16px; padding: 0 12px; }
            .stay-card .card-body { padding: 18px; }
            .stay-card .card-image { height: 180px; }
            .stay-card .card-body h3 { font-size: 1.1rem; }
            .stay-card .card-body .card-price .amount { font-size: 1.3rem; }
            .stay-card .card-body .card-actions .btn { font-size: 0.78rem; padding: 8px 12px; }
            .delete-modal { padding: 32px 24px 28px; }
            .delete-modal .modal-actions { flex-direction: column; }
        }
        @media (max-width: 576px) {
            .stay-grid .grid-wrapper { grid-template-columns: 1fr; max-width: 400px; margin: 0 auto; }
            .stay-card .card-image { height: 200px; }
            .stats-bar .stats-wrapper { gap: 20px; }
            .stats-bar .stat-item .stat-number { font-size: 1.4rem; }
            .page-header h1 { font-size: 1.5rem; }
            .page-header p { font-size: 0.9rem; }
            .delete-modal { padding: 28px 20px 24px; }
        }
    </style>
</head>
<body>
        ...
        <div class="grid-wrapper" id="stayGrid">
                        ...
                        <div class="stay-card" data-aos="fade-up" data-aos-delay="<%= (count % 4) * 100 + 100 %>">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-actions">
                                    <a href="updateStay.jsp?id=<%= id %>" class="btn btn-update">
                                        <i class="fa-regular fa-pen-to-square"></i> Update
                                    </a>
                                    <button type="button" class="btn btn-delete" 
                                            onclick="openDeleteModal(<%= id %>, '<%= name %>', 'stay')">
            ...
            %>
                        <div class="empty-state">
                            ...
                            <p>Start by adding your first accommodation option.</p>
                            <a href="add_stay.jsp" class="btn-add">
...
<div class="delete-modal-overlay" id="deleteModal">
    <div class="delete-modal">
        ...
        <p style="font-size: 0.85rem; color: #6b7a6b;">This action cannot be undone.</p>
        <span class="item-name" id="deleteItemName">Item Name</span>
        <div class="modal-actions">
            <button class="btn btn-modal-cancel" onclick="closeDeleteModal()">
                <i class="fa-solid fa-xmark"></i> Cancel
            </button>
            <button class="btn btn-modal-confirm" id="confirmDeleteBtn">
...
<script>
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
        if (deleteItemId && deleteItemType) {
            ...
            form.method = 'POST';
            form.action = deleteItemType === 'stay' ? 'StayAction' : 
                          deleteItemType === 'safari' ? 'SafariAction' :
                          deleteItemType === 'package' ? 'PackageAction' : 'GuideAction';
            ...
            idInput.type = 'hidden';
            idInput.name = deleteItemType === 'stay' ? 'stayId' :
                          deleteItemType === 'safari' ? 'safariId' :
                          deleteItemType === 'package' ? 'packageId' : 'guideId';
