...
<section class="page-header">
    <div class="container">
        <div class="header-content" data-aos="fade-up">
            ...
            <h1>Trip <span>Management</span></h1>
            <p>Manage all bookings and trip requests</p>
        </div>
    </div>
</section>
        ...
        <div class="row g-4">
            <%
                ...
                try {
                    ...
                    con = DriverManager.getConnection("jdbc:mysql://localhost:3306/wildlife", "root", "ganesh@123");
                    ...
                    if (rs.next()) {
                        ...
                        completedBookings = rs.getInt("completed");
...
<!-- Table Section -->
<section class="table-section" style="padding: 20px 0 40px;">
    <div class="container">
        <div class="table-container" data-aos="fade-up">
            <div class="table-header">
                <h2><i class="fa-regular fa-list"></i> All Bookings</h2>
                    ...
                    <tbody>
                        <%
                            try {
                                ...
                                while (rs.next()) {
                                    ...
                                    if ("completed".equalsIgnoreCase(status)) statusClass = "completed";
                                    else if ("cancelled".equalsIgnoreCase(status)) statusClass = "cancelled";
                                    else if ("confirmed".equalsIgnoreCase(status)) statusClass = "confirmed";
                        %>
                                    <tr>
                                        <td><strong>#<%= bookingId %></strong></td>
                                        <td>
                                            <div><strong><%= rs.getString("tourist_name") %></strong></div>
                                            <small style="color: #6b7a6b; font-size: 0.75rem;">ID: <%= rs.getInt("user_id") %></small>
                                        </td>
                                        <td>
                                            <div><strong><%= rs.getString("item_name") %></strong></div>
                                            <small style="color: #6b7a6b; font-size: 0.7rem; text-transform: capitalize;"><%= rs.getString("item_type") %></small>
                                        </td>
                                        <td>
                                            <img src="<%= rs.getString("item_image") %>" class="item-image" alt="Item" 
                                                 onerror="this.src='https://placehold.co/70x50/1a2e1a/ffffff?text=Item'">
                                        </td>
                                        <td><%= rs.getInt("num_persons") %></td>
                                        <td><strong>₹<%= String.format("%.2f", rs.getDouble("total_price")) %></strong></td>
                                        <td><span style="font-size: 0.8rem;"><%= rs.getString("payment_mode") %></span></td>
                                        <td><span class="status-badge <%= statusClass %>"><%= status %></span></td>
                                        <td style="font-size: 0.8rem;"><%= rs.getTimestamp("booked_date") %></td>
                                        <td>
                                            <div class="action-buttons">
                                                <%
                                                    if (!"cancelled".equalsIgnoreCase(status) && !"completed".equalsIgnoreCase(status)) {
                                                %>
                                                    <form method="post" action="updateBookingStatus" style="display:inline;">
                                                        <input type="hidden" name="bookingId" value="<%= bookingId %>">
                                                        <button name="action" value="completed" class="btn-action btn-done" 
                                                                onclick="return confirm('Mark this booking as completed?')">
                                                            <i class="fa-solid fa-check"></i> Done
                                                        </button>
                                                        <button name="action" value="cancelled" class="btn-action btn-cancel"
                                                ...
                                                %>
                                                    <span class="btn-action btn-closed">
                        ...
                        %>
                                <tr>
                                    <td colspan="11">
                                        <div class="empty-state">
                                            <div class="empty-icon" style="color: #dc2626;"><i class="fa-regular fa-circle-exclamation"></i></div>
                                            <h3 style="color: #dc2626;">Error Loading Bookings</h3>
                                            <p><%= e.getMessage() %></p>
                                        </div>
                                    </td>
                                </tr>
